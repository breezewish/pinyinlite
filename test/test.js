...
function normalizePinyin(pinyin) {
  return pinyin.split('').map((symbol) => {
...
describe('pinyinlite', () => {
  ...
  it('should output pinyin for common simplified characters', () => {
    const expected = [
    ...
    const result = pinyinlite(test);
    expected.split('/').forEach((pinyin, index) => {
      if (pinyin !== '') {
  ...
  it('should output pinyin for common traditional characters', () => {
    const expected = [
  ...
  it('should output all candidate pinyin for heteronyms', () => {
    const dictContent = fs.readFileSync(`${__dirname}/../dev/dict.txt`).toString().split('\n');
    dictContent.forEach((line) => {
      if (line.indexOf(' ') === -1) {
        return;
      }
      const p = line.split(' ');
      if (p.length === 2 && p[1].length > 0) {
        const result = pinyinlite(p[1]);
        const pinyin = p[0].replace(/\d/g, '');
        result.forEach((r, i) => r.should.containEql(pinyin));
      }
    });
  });
